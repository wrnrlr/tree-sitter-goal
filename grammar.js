module.exports = grammar({
  ...
  rules: {
    ...
    atom: $ => choice($.nil, $.infinity, $.smallest, $.binairy, $.int, $.number, $.hex, $.exponential, $.duration, $.name, $.string, $.qq_string, $.rq_string, $.rx_string),
    ...
    smallest: _ => /0i/i,
    qq_string: _ => choice(
      seq('qq', token.immediate('/'), repeat(/[^/]/), '/'),
      seq('qq', token.immediate('#'), repeat(/[^#]/), '#'),
      seq('qq', token.immediate('`'), repeat(/[^`]/), '`'),
      seq('qq', token.immediate('"'), repeat(/[^"]/u), '"'),
      seq('qq', token.immediate("'"), repeat(/[^']/u), "'"),
      seq('qq', token.immediate('%'), repeat(/[^%]/), '%')
    ),
    rq_string: _ => choice(
      seq('rq', token.immediate('/'), repeat(/[^/]/), '/'),
      seq('rq', token.immediate('#'), repeat(/[^#]/), '#'),
      seq('rq', token.immediate('`'), repeat(/[^`]/), '`'),
      seq('rq', token.immediate('"'), repeat(/[^"]/u), '"'),
      seq('rq', token.immediate("'"), repeat(/[^']/u), "'"),
      seq('rq', token.immediate('%'), repeat(/[^%]/), '%')
    ),
    rx_string: _ => choice(
      seq('rx', token.immediate('/'), repeat(/[^/]/), '/'),
      seq('rx', token.immediate('#'), repeat(/[^#]/), '#'),
      seq('rx', token.immediate('`'), repeat(/[^`]/), '`'),
      seq('rx', token.immediate('"'), repeat(/[^"]/u), '"'),
      seq('rx', token.immediate("'"), repeat(/[^']/u), "'"),
      seq('rx', token.immediate('%'), repeat(/[^%]/), '%')
    ),
